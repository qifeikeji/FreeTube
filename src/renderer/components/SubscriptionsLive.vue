<template>
  <SubscriptionsTabUi
    ...
    :last-refresh-age-ms="lastLiveRefreshAgeMs"
    :title="t('Global.Live')"
...
const lastLiveRefreshAgeMs = computed(() => {
  if (lastRemoteRefreshSuccessTimestamp.value) {
...
async function loadVideosForSubscriptionsFromRemote() {
  ...
  const videoListFromRemote = (await Promise.all(channelsToLoadFromRemote.map(async (channel) => {
    try {
      ...
      console.error('[SubscriptionsLive] Failed to load subscription channel', channel, err)
